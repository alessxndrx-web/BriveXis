...
import { searchWorkspace, type SearchResult } from '../dealer.selectors';
import { paths } from '../dealer.routes';
import { useRouter } from '../../../lib/router';

/**
 * Global search across the records a dealership looks things up by.
 *
...
const HREFS = {
  ...
  deal: paths.deal,
  financing: paths.financingApplication,
};
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <label htmlFor="workspace-search" className="sr-only">
        Search leads, customers, inventory, reservations and deals
      </label>
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search stock number, VIN, customer…"
          value={query}
          onChange={(event) => {
        ...
        >
          {results.length === 0 ? (
            ...
            results.map((result, index) => (
              ...
              >
                <span className="text-[0.625rem] font-semibold uppercase tracking-[0.1em] text-copper w-[4.5rem] shrink-0">
                  {result.group}
                </span>
