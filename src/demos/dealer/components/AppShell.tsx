...
import { MODULES, MODULE_GROUPS, paths, type ModuleId } from '../dealer.routes';
 ...
 *
 * It shares its structure with the contractor demo — the same navigation
 * behaviour, the same demo banner, the same reset flow — because both are the
 * same product family. What differs is what the navigation contains, which is
 * where a dealership and a field-service business genuinely diverge.
 */
...
export function AppShell({ activeModule, children }: AppShellProps) {
  const { state, role, setRole, resetDemo, notify } = useDealer();
  ...
  const sidebarFooter = (
    <div className="border-t border-dark-border px-3 py-3 space-y-3">
      ...
      <a
        href={CONTACT_ANCHOR}
        onClick={() => track('dealer_demo_cta_clicked')}
    ...
    <div className="min-h-screen bg-app-bg">
      ...
      >
        <div className="flex h-full items-center gap-2 sm:gap-3 px-3 sm:px-4">
          ...
          <a
            ...
            aria-label="Dealer Operations — overview"
          >
      ...
      <ConfirmDialog
        ...
        description={
          <>
            <p>
              Every change you have made in this demo — new leads, customers, reservations,
              deals, financing records and payments — will be discarded and the workspace
              restored to its original sample data.
            </p>
 ...
 * but present on every screen so nobody can mistake the data for a real dealer.
 */
  ...
  return (
    ...
    >
      ...
      <span className="text-[0.75rem] text-muted">
        {companyName} is a fictional dealership. All business, customer and vehicle data shown
        here is fictional.
      </span>
    </div>
  );
