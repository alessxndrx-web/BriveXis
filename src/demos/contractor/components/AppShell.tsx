...
import { MODULES, MODULE_GROUPS, paths, type ModuleId } from '../contractor.routes';
import { useContractor } from '../ContractorProvider';
import { ROLE_MODULES } from '../contractor.selectors';
import type { DemoRole } from '../contractor.types';
 ...
 *
 * Desktop keeps a fixed sidebar so navigation never scrolls away. Mobile
 * replaces it with a slide-over that traps nothing the visitor needs — the
 * same links, reachable from a single button.
 */
...
export function AppShell({ activeModule, children }: AppShellProps) {
  const { state, role, setRole, resetDemo, notify } = useContractor();
  ...
  const sidebarFooter = (
    <div className="border-t border-dark-border px-3 py-3 space-y-3">
      ...
      <a
        href={CONTACT_ANCHOR}
        onClick={() => track('contractor_demo_cta_clicked')}
    ...
    <div className="min-h-screen bg-app-bg">
      ...
      >
        <div className="flex h-full items-center gap-2 sm:gap-3 px-3 sm:px-4">
          ...
          <a
            ...
            aria-label="Contractor Operations — overview"
          >
      ...
      <ConfirmDialog
        ...
        description={
          <>
            <p>
              Every change you have made in this demo — new leads, estimates, jobs, invoices
              and payments — will be discarded and the workspace restored to its original
              sample data.
            </p>
  ...
  return (
    ...
    >
      ...
      <span className="text-[0.75rem] text-muted">
        {companyName} is a fictional company. All company and customer data shown here is
        fictional.
      </span>
    </div>
  );
